...
// 		if (preg_match("/^13[0-9]{1}[0-9]{8}$|15[01236789]{1}[0-9]{8}$|18[01236789]{1}[0-9]{8}$/", $mobilephone)) {
	...
	public function sendSms($data = array(), $send_time = '', $charset = 'utf-8', $id_code = '')
	{
		...
		if ($data) {
			...
			$sysconfig=C('SMS_CONFIG');
			     
			$smsapi_senturl =  $sysconfig['url'];
			$site_name=$sysconfig['username'];
			$sms_uid = $sysconfig['apikey'];
			$sms_pwd = $sysconfig['password'];

			$data = array
			(
			'username'=>$sysconfig['username'],		//用户账号
			'password'=>$sysconfig['password'],		//密码
			'apikey'=>$sysconfig['apikey'],			//apikey
			'mobile'=>$mobile,					//号码
			'content'=>$content,				//内容
			);
			$url=$sysconfig['url'];
			...
			// $result= Sms::curlSMS($url,$data);			//POST方式提交
			//成功返回success+id 不成功返回error+信息
			...
			//增加到本地数据库
			$row = array('pigcms_id'=> $hq_id,'mer_id' => $mer_id, 'uid' => $uid, 'store_id' => $store_id, 'time' => $send_time, 'phone' => $mobile, 'text' => $content, 'status' =>$sms_status, 'type' => $type, 'sendto' => $sendto);
