...
class Weixin_bindModel extends Model
{
	public function get_access_token($mer_id){
		$weixin_bind = $this->field(true)->where(array('mer_id' => $mer_id))->find();
		...
		if ($weixin_bind['authorizer_access_token'] && $weixin_bind['authorizer_access_token_expires'] > time()) {
			...
			if ($access_data['errcode']) {
				...
				$this->where(array('mer_id' => $mer_id))->data(array('authorizer_access_token'=>$access_data['authorizer_access_token'],'authorizer_access_token_expires'=>$access_data['expires_in'] + time()))->save();
			}
		}
