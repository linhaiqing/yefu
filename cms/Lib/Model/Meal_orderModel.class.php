...
// 			return array('error'=>1,'msg'=>'该订单已付款！','url'=>U('User/Index/meal_order_view',array('order_id' => $now_order['order_id'])));
			...
			//判断会员卡余额
			$merchant_balance = floatval($order_param['merchant_balance']);
			if($merchant_balance){
			...
			//判断帐户余额
			$balance_pay = floatval($order_param['balance_pay']);
			if($balance_pay){
			...
			if($this->where($where)->save($data_meal_order)){
				...
				if ($now_user['openid'] && $order_param['is_mobile']) {
					...
					if ($order_param['pay_type'] == 'offline' && empty($order_param['third_id'])) {
						$model->sendTempMsg('OPENTM201752540', array('href' => $href, 'wecha_id' => $now_user['openid'], 'first' => C('config.meal_alias_name').'提醒', 'keyword2' => $now_order['order_id'], 'keyword1' => $keyword2, 'keyword3' => $now_order['price'], 'keyword4' => date('Y-m-d H:i:s'), 'remark' => '购买成功（线下未支付），您的消费码：'.$data_meal_order['meal_pass']));
					} else {
						$model->sendTempMsg('OPENTM201752540', array('href' => $href, 'wecha_id' => $now_user['openid'], 'first' => C('config.meal_alias_name').'提醒', 'keyword2' => $now_order['order_id'], 'keyword1' => $keyword2, 'keyword3' => $now_order['price'], 'keyword4' => date('Y-m-d H:i:s'), 'remark' => '购买成功，您的消费码：'.$data_meal_order['meal_pass']));
					}
				}
	...
	public function get_order_by_mer_id($mer_id, $is_system = false)
	{
		...
		$sql = "SELECT sum(balance_pay + payment_money) as price, is_pay_bill FROM ". C('DB_PREFIX') . "meal_order WHERE mer_id={$mer_id} AND paid=1 AND status in (1,2) AND (pay_type<>'offline' OR balance_pay<>'0.00') GROUP BY is_pay_bill";
		$sql .= ' UNION ALL ';
		$sql .= "SELECT sum(balance_pay + payment_money) as price, is_pay_bill FROM ". C('DB_PREFIX') . "group_order WHERE mer_id={$mer_id} AND paid=1 AND status in (1,2) AND (pay_type<>'offline' OR balance_pay<>'0.00') GROUP BY is_pay_bill";
		$res = $mode->query($sql);
		...
		$sql = "SELECT 1 as name, order_id, info as order_name, uid, mer_id, store_id, phone, total, (balance_pay+payment_money) as price, price as order_price, dateline, paid, status, pay_type, pay_time, third_id, is_mobile_pay, balance_pay, payment_money, card_id, merchant_balance, is_pay_bill FROM ". C('DB_PREFIX') . "meal_order WHERE mer_id={$mer_id} AND paid=1 AND status in (1,2) AND (pay_type<>'offline' OR balance_pay<>'0.00')";
		$sql .= ' UNION ALL ';
		$sql .= "SELECT 2 as name, order_id, order_name, uid, mer_id, store_id, phone, num as total, (balance_pay+payment_money) as price, total_money as order_price, add_time as dateline, paid, status, pay_type, pay_time, third_id, is_mobile_pay, balance_pay, payment_money, card_id, merchant_balance, is_pay_bill FROM ". C('DB_PREFIX') . "group_order WHERE mer_id={$mer_id} AND paid=1 AND status in (1,2) AND (pay_type<>'offline' OR balance_pay<>'0.00')";
		...
		foreach ($res as &$l) {
			...
			$l['order_price'] = number_format($l['order_price'],2);
		}
		$pagebar = $p->show();
