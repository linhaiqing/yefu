...
class LoginAction extends BaseAction {
    public function index() {
        if($this->user_session)$this->redirect("Index/index");
        if (IS_POST) {
            ...
            $hq_assign['referer'] = !empty($_GET['referer']) ? strip_tags(htmlspecialchars_decode($_GET['referer'])) : (!empty($_SERVER['HTTP_REFERER']) ? strip_tags($_SERVER['HTTP_REFERER']) : U('Index/Index/index'));
            $hq_assign['url_referer'] = urlencode($hq_assign['referer']);
            $this->assign($hq_assign);
            $this->display();
        }
    }
    public function cdsso(){
        $accesstoken=I('accesstoken');
        if($accesstoken){
            $result=D('User')->checksiglelogin($accesstoken);
            if($result)$this->success("ok");
        }
    }
    public function singlelogin(){     
            
       if(I('referer'))session('referer',$_SERVER['HTTP_REFERER']);
         
        $UserEvent = A('User','Event');
        $UserEvent->_initialize();
        $code=I('code');
        if(!$code){
            $callback = $this->config['site_url'].'/wap.php?c=Login&a=singlelogin';
            $UserEvent->getUserCode($callback);
        }
        $AccessToken=$UserEvent->getAccessTokenByCode($code);
        $userinfo=$UserEvent->getUserInfoByAccessToken($AccessToken['access_token']);
        if($userinfo['nFlag']==1){
            $userinfo=$userinfo['Data'];
            $result=D('User')->singlelogin($userinfo);
            if (empty($result['error_code'])) {
                session('user', $result['user']);
    ...
    public function singlereg(){
        $callback = $this->config['site_url'].'/index.php?c=Login&a=singlelogin';
        $callback=urlencode($callback);
        $url=C('ZYACCOUNT_URL')."/index.php/Login/userregister.html?callback=".$callback;
        redirect($url);
    ...
    public function reg() {
        if (IS_POST) {
            ...
            $result = D('User')->checkreg($phone, $pwd,$vfycode);
    ...
    public function Generate() {
        ...
        if ($Userarr || $salt['nFlag']==1)
         exit(json_encode(array('error_code' => 3, 'msg' => '手机账号已存在，快去登录吧！')));
        $user_modifypwdDb = M('User_modifypwd');
        $modifypwd = $user_modifypwdDb->where("telphone='$phone'")->order('id desc')->getfield('addtime');
        if($modifypwd+60>time())exit(json_encode(array('error_code' => 1, 'msg' => '一分钟之内只能发送一条短信')));
            $vcode = createRandomStr(6,true);
            $content = '您的验证码是：' . $vcode . '。此验证码20分钟内有效，请不要把验证码泄露给其他人。如非本人操作，可不用理会！';
            $test=Sms::sendSms(array('mer_id' => 1, 'store_id' => 0, 'content' => $content, 'mobile' => $phone, 'uid' => 0, 'type' => 'mfypwd'));
            $addtime = time();
    ...
    public function logout() {
        ...
        $this->success('退出成功！');
        $hq_referer = 'http://z1/quweizhichang/index.php?c=Login&a=logout';
        $url='http://account.com/index.php/login/updateUserCookies?callback='.$hq_referer;
        // if(1==1){
        //     redirect($url);
        // }
        // $this->success('登录成功！');
    ...
    public function frame_login() {
        $hq_assign['referer'] = !empty($_GET['referer']) ? strip_tags($_GET['referer']) : (!empty($_SERVER['HTTP_REFERER']) ? strip_tags($_SERVER['HTTP_REFERER']) : U('Index/Index/index'));
        $hq_assign['url_referer'] = urlencode($hq_assign['referer']);
        $this->assign($hq_assign);
        $this->display();
    }
