<include file="Public:header"/>
	<form id="myqcorm" method="post" action="{:U('Merchant/news_amend')}" frame="true" refresh="true">
		<input type="hidden" name="id" value="{$now_news.id}"/>
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			<tr>
				<th width="80">标题</th>
				<td><input type="text" class="input fl" name="title" value="{$now_news.title}" size="75" placeholder="公告标题" validate="maxlength:50,required:true"/></td>
			</tr>
			<tr>
				<th width="80">内容</th>
				<td>
					<textarea name="content" id="content">{$now_news.content}</textarea>
				</td>
			</tr>
			<tr>
				<th width="80">置顶</th>
