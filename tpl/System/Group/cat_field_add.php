<include file="Public:header"/>
	<div id="nav" class="mainnav_title">
		<ul>
			<a href="{:U('Group/cat_field_add',array('cat_id'=>$_GET['cat_id'],'type'=>'0'))}" <if condition="$_GET['type'] eq 0">class="on"</if>>自定义字段</a>|
			<a href="{:U('Group/cat_field_add',array('cat_id'=>$_GET['cat_id'],'type'=>'1'))}" <if condition="$_GET['type'] eq 1">class="on"</if>>选择内置字段</a>
		</ul>
	</div>
	<form id="myqcorm" method="post" action="{:U('Group/cat_field_modify')}" frame="true" refresh="true">
