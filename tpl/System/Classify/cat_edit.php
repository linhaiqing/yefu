<include file="Public:header"/>
	<form id="myqcorm" method="post" action="{:U('Classify/cat_amend')}" enctype="multipart/form-data">
		<input type="hidden" name="cid" value="{$now_category['cid']}"/>
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			<tr>
				<th width="80">分类名称</th>
				<td><input type="text" class="input fl" name="cat_name" id="cat_name" onBlur="TextSEO(this.value)" value="{$now_category.cat_name}" size="25" placeholder="" validate="maxlength:20,required:true" tips=""/></td>
			</tr>
			<tr>
				<th width="80">短标记(url)</th>
				<td><input type="text" class="input fl" name="cat_url" id="cat_url" value="{$now_category.cat_url}" size="25" placeholder="英文或数字" validate="maxlength:20,required:true,en_num:true" tips="只能使用英文或数字，用于网址（url）中的标记！建议使用分类的拼音"/></td>
			</tr>
			<tr>
				<th width="80">是否热门</th>
				<td>
			...
			<if condition="!empty($now_category['cat_pic'])">
				<tr>
					<th width="80">分类现图</th>
					<td><img src="{$config.site_url}/upload/system/{$now_category.cat_pic}" style="width:50px;height:50px;" class="view_msg"/></td>
				</tr>
			</if>
			<tr>
			...
			<tr>
				<th width="80">分类排序</th>
				<td><input type="text" class="input fl" name="cat_sort" value="{$now_category.cat_sort}" size="10" placeholder="分类排序" validate="maxlength:6,required:true,number:true" tips="默认添加时间排序！手动排序数值越大，排序越前。"/></td>
			</tr>
		...
		<td><input type="text" tips="一般不超过80个字符！" validate="" style="width:260px;"  id="seo_title" name="seo_title" class="input fl" value="{$now_category.seo_title}"></td></tr>
		<tr><th width="80">SEO关键词：</th><td><input type="text" tips="一般不超过100个字符！" validate="" style="width:260px;"  id="seo_keywords" name="seo_keywords" class="input fl" value="{$now_category.seo_keywords}"></td></tr>
		<tr><th width="80">SEO描述：</th><td><textarea tips="一般不超过200个字符！" validate="" id="seo_description" name="seo_description" style="width:250px;height:90px;">{$now_category.seo_description}</textarea></td></tr>
		</table>
