<include file="Public:header"/>
	<div id="nav" class="mainnav_title">
		<ul>
			<a href="{:U('Classify/cat_field_add',array('cid'=>$_GET['cid'],'type'=>'0'))}" <if condition="$_GET['type'] eq 0">class="on"</if>>自定义字段</a>|
			<a href="{:U('Classify/cat_field_add',array('cid'=>$_GET['cid'],'type'=>'1'))}" <if condition="$_GET['type'] eq 1">class="on"</if>>选择内置字段</a>
		</ul>
	</div>
					...
					<td>
						<select name="type" onchange="ShowTextarea(this.value);">
						   <if condition="!empty($inputTypeArr)">
								...
								<option value="{$vo['typ']}">{$vo['tname']}</option>
								</volist>
							</if>
						</select>
