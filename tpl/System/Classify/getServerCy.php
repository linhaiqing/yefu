<include file="Public:header"/>
   ...
   .desctitle{font-size: 16px;margin: 5px 0 0 15px;}
    ...
    <div class="desctitle">请给 <span style="color:red;">{$param['cyname']}</span> 选择对应更新二级分类</div>
	...
	<form id="myqcorm" name="myqcorm" method="post" action="{:U('Classify/savepicksetCy')}" frame="true" refresh="true">
	<div id="popCyDiv">
	  <div id="Cymain">
	  <span>请点击选择一级分类：</span>
	  ...
	  <div id="Cysub"> 
	  <span>请点击选择二级分类：</span>
	  <select size="15" onclick="GetFiledSelect(this.value,{$pickset['mcid']})">
	   <volist name="subcy[$pickset['mcid']]" id="mvv">
	      <option value="{$mvv['cid']}" <if condition="$pickset['mscid'] eq $mvv['cid']"> selected="selected"</if>>{$mvv['szhname']}</option>
	   	  ...
	   	  <div id="Cyqcield">
		     <span>{$subcy[$pickset['mcid']][$pickset['mscid']]['szhname']} 所有字段：</span>
			 <select size="15">
		   	   <volist name="subcy[$pickset['mcid']][$pickset['mscid']]['fields']" id="fvv">
				<option value="{$key}">{$fvv}</option>
			</volist>
			</select>
		  </div>
	   <else/>
		  <div id="Cysub" style="display:none;"> </div>
	   	  <div id="Cyqcield" style="display:none;"> </div>
	  </if>
...
function SubmitForm(){
   document.getElementById('myqcorm').submit();
}
</script>
