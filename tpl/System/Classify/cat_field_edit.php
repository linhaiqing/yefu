<include file="Public:header"/>
	...
	<form id="myqcorm" method="post" action="{:U('Classify/cat_field_modify')}" frame="true" refresh="true">
		<input type="hidden" name="cid" value="{$cid}"/>
		<input type="hidden" name="numfilter" value="{$isfilter}"/>
		<input type="hidden" name="fkey" value="{$fkey}"/>
		<if condition="!isset($thiscat_field['use_field'])">
			<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
				<tr>
					<th width="80">字段名称</th>
					<td><input type="text" class="input fl" name="name" id="name" size="25" value="{$thiscat_field['name']}" placeholder="" validate="maxlength:20,required:true" tips=""/></td>
				</tr>
				<tr>
					<th width="80">短标记(url)</th>
					<td><input type="text" class="input fl" name="url" id="url" size="25" value="{$thiscat_field['url']}" placeholder="英文或数字" validate="maxlength:20,required:true,en_num:true" tips="只能使用英文或数字，用于网址（url）中的标记！建议使用字段的拼音"/></td>
				</tr>
				<tr>
					<th width="80">是否必填</th>
					<td>
					...
					<td>
						<select name="type" onchange="ShowTextarea(this.value);">
						   <if condition="!empty($inputTypeArr)">
								...
								<option value="{$vo['typ']}" <php>if($thiscat_field['type']==$vo['typ']) echo 'selected="selected"';</php>>{$vo['tname']}</option>
								</volist>
							</if>
						</select>
	   ...
	   <select name="inunit" <php>if($thiscat_field['inarr']!=1) echo 'style="display:none"';</php> id="input_unit">
			...
			<option value="元/㎡" <php>if($thiscat_field['inunit']=='元/㎡') echo 'selected="selected"';</php>>元/㎡</option>
			  ...
			  <tr id="valueoftype" <php>if($thiscat_field['type']==1 || $thiscat_field['type']==5) echo 'style="display:none;"'</php>>
				  <th width="80">供选择值</th>
				  <td><textarea class="input fl" style="width:250px;height:80px;" name="valueoftype" placeholder ="必须写上这个字段类型供发布者选择的值一行算一个值">{$optstr}</textarea></td>
			   </tr>
				...
				<tr id="myonoffShow" <php>if(empty($textstr)) echo 'style="display:none;"';</php>>
				  ...
				  <td><textarea class="input fl" style="width:250px;height:80px;" name="filtercon" placeholder ="即这个字段的可能筛选条件一行算一个筛选条件">{$textstr}</textarea><span class="red">(前台支持：字段类型为单文本框且属性为只填写数字情况下 这里可以写xxx-xxxx这样数字区间格式)</span></td>
			   </tr>
			   <else/>
			    <php>if($isfilter <= 4){
