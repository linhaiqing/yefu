<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{:U('Home/other')}">关键词回复列表</a>|
					<a href="{:U('Home/other_edit',array('id'=>$info['id']))}" class="on">编辑关键词回复</a>
				</ul>
			</div>
			<form method="post" action="" refresh="true" enctype="multipart/form-data" >
				<table cellpadding="0" cellspacing="0" class="table_form" width="100%">
					<tr>
						<th width="80">关键词</th>
						<td>
						<input type="text" class="input-text" name="key" size="20" placeholder="关键词" validate="maxlength:20,required:true" value="{$info.key}"/><span style="color: red;">使用相同的关键词可以实现多图文</span>
						<input type="hidden" name="id" value="{$info.id}">
						</td>
					</tr>
					<tr>
						<th width="80">回复标题</th>
						<td><input type="text" class="input-text" name="title" size="20" placeholder="回复标题" validate="maxlength:20,required:true" value="{$info.title}"/></td>
					</tr>
					<tr>
						<th width="80">文章简介</th>
						<td><textarea rows="6" cols="104" name="info" >{$info.info}</textarea></td>
					</tr>
					<tr>
						<th width="80">回复内容</th>
						<td><textarea rows="" cols="" name="content" id="content">{$info.content}</textarea></td>
					</tr>
					<tr>
						<th width="80">外链接url</th>
						<td>
							<input type="text" class="input-text" name="url" id="url" style="width:200px;" placeholder="外链接url" validate="maxlength:200,url:true" value="{$info.url}"/>
					...
					<tr>
						<th width="80">回复图片</th>
						<td>
							<input type="file" class="input-text" name="pic" id="pic" value="{$info.pic}"/>
						</td>
					</tr>
					<if condition="$info['pic']">
						<tr>
							<th width="160"></th>
							<td><img src="{$info.pic}" width="280" height="180"/></td>
						</tr>
					</if>
					<tr>
						<th width="80">第三方接口url</th>
						<td>
							<input type="text" class="input-text" name="api_url" id="api_url" style="width:200px;" placeholder="第三方接口url" value="{$info.api_url}"/>
