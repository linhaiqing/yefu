<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{:U('User/levellist')}" class="on">等级管理</a>
					<a href="javascript:void(0);" onclick="window.top.artiframe('{:U('User/addlevel')}','添加一个等级',650,500,true,false,false,addbtn,'add',true);" style="margin-left:20px;">添加等级</a>
				</ul>
			</div>
							...
							<if condition="is_array($userlevel)">
								<volist name="userlevel" id="vo">
									<tr>
										...
										<td>{$vo.lname}</td>
										<td>{$vo.level}</td>
										<td><img src="{$config['site_url']}{$vo.icon}" style="width:90px; height: 80px;"></td>
										<td><if condition="$vo['type'] eq 1">商品按原价{$vo.boon}%计算<elseif condition="$vo['type'] eq 2" />商品价格立减{$vo.boon}元<else />无</if></td>
										<td><a href="javascript:void(0);" onclick="window.top.artiframe('{:U('User/addlevel',array('lid'=>$vo['id']))}','编辑等级信息',650,500,true,false,false,editbtn,'edit',true);">编 辑</a></td>
									</tr>
								</volist>
