<include file="Public:header"/>
		<div class="mainbox">
				...
				<div id="nav" class="mainnav_title">
					<ul>
						<volist name="group_list" id="vo">
							<a href="{:U('Config/index',array('gid'=>$vo['gid']))}" <if condition="$gid eq $vo['gid']">class="on"</if>>{$vo.gname}</a>|
						</volist>
					</ul>
				</div>
			<else/>
			...
			<form id="myqcorm" method="post" action="{:U('Config/amend')}" refresh="true">
				{$config_tab_html}
				{$config_html}
				<div class="btn" style="margin-top:20px;">
					...
					<if condition="empty($_GET['galias'])">
						...
						<input type="button"  value="微信API接口填写信息" class="button" onclick="window.top.artiframe('{:U('Config/show',array('id'=>$vo['id']))}','API接口信息',560,100,true,false,false,'','add',true);"/>
		...
		<script>
			$(function(){
				...
				$('#live_service_key').click(function(){
					window.top.msg(2,'正在请求中,请稍等...',true,100);
					$.get("{:U('Config/live_service')}",function(data){
						if(data.error_code){
							window.top.msg(0,data.msg,true,3);
						}else{
							window.top.msg(1,data.msg,true,3);
		...
		<script type="text/javascript">
			KindEditor.ready(function(K){
				...
				$('.config_upload_image_btn').click(function(){
					...
					editor.uploadJson = "{:U('Config/ajax_upload_pic')}";
				...
				$('.config_upload_file_btn').click(function(){
					var upload_file_btn = $(this);
					editor.uploadJson = "{:U('Config/ajax_upload_file')}&name="+upload_file_btn.siblings('.input-file').attr('name');
