<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="javascript:void(0);" class="on" onclick="window.top.artiframe('{:U('Card/create_add', array('id' => $thisCard['id']))}','创建会员卡号',560,350,true,false,false,addbtn,'add',true);">创建会员卡号</a>|
						...
						<tbody>
							<if condition="$data_vip">
									<volist name="data_vip" id="data_vip">
										<tr >
											<td><input type="checkbox" value="{$data_vip.id}" class="cbitem" name="id_{$i}"></td>
											<td>{$data_vip.number}</td>
											<td><if condition="$data_vip['wecha_id'] eq false">空闲卡<else/><strong>使用中</strong></if></td>
											<td><a href="admin.php?g=System&c=Card&a=members&itemid={$data_vip.id}&id={$thisCard.id}">查看详情</a></td>
										</tr>
									</volist>
								<else/>
