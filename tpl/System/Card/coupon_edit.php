...
'insertunorderedlist', '|','emoticons', 'link', 'unlink']
		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<a href="{:U('Index/pass')}" class="on">{$thisCard.cardname}：发布优惠券</a>
			</div>
			<form id="myqcorm" method="post" action="" refresh="true" enctype="multipart/form-data" >
				<table cellpadding="0" cellspacing="0" class="table_form" width="100%">
					<tr>
						<td width="100">券名称：</td>
						<td><input type="text" class="input-text" value="{$vip.title}" name="title"/></td>
					</tr>
					<tr>
						<td>有效期：</td>
						<td>
							<input type="text" class="input-text" id="statdate" value="{$vip.statdate|date='Y-m-d',###}" onClick="WdatePicker()" name="statdate" style="width:90px;"> （含）到
							<input type="text" class="input-text" id="enddate" value="{$vip.enddate|date='Y-m-d',###}" name="enddate" onClick="WdatePicker()" style="width:90px;">（含）
						</td>
					</tr>
					<tr>
						<td>优惠劵属性：</td>
					...
					<tr>
						<td>抵用金额：</td>
						<td>
							<input type="input" class="input-text"  value="{$vip.price}" name="price" style="width:50px;"> 元
						</td>
					</tr>
					<tr>
						<td>优惠劵图标：</td>
						<td>
							<img src="<if condition="$vip.pic eq '' and $vip.type eq 0">/static/images/cart_info/daijin.png<else/>{$vip.pic}</if>" id="pic_src"style="max-width:200px;"><br/>
							<input type="text" name="pic" id="pic" value="<if condition="$vip.pic eq '' and $vip.type eq 0">/static/images/cart_info/daijin.png<else/>{$vip.pic}</if>" class="input-text" style="width:200px;"> 
					...
					<tr>
						...
						<td>
							<textarea name="info" id="info" rows="5" style="width: 410px; height: 250px; display: none;">{$vip.info}</textarea>
						</td>
					</tr>  
					<tr>
						<td>数量：</td>
						<td>每个用户可以获得：<input type="input" class="input-text" id="people" value="{$vip.people}" name="people" style="width:50px;"> 张券</td>
					</tr>
				</table>
				<div class="btn">
