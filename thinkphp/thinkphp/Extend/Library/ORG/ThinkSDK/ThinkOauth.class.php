...
abstract class ThinkOauth{
	...
	public function __construct($token = null){
		...
		if(empty($config[$this->Type.'KEY']) || empty($config[$this->Type.'Secret'])){
			throw new \Exception('请配置您申请的APP_KEY和APP_SECRET');
		} else {

			$this->AppKey    = $config[$this->Type.'KEY'];
            $this->AppSecret = $config[$this->Type.'Secret'];
            $this->Token     = $token; //设置获取到的TOKEN
            $this->heards = $this->getHeaderInfo();
		}
	}
	function  getHeaderInfo(){
		...
		array_push($_headers, "CLIENTID: ".$this->AppKey);
		array_push($_headers, "TIME: " . time());
		return $_headers;
	}
	/**
     * 取得Oauth实例
    ...
    public static function getInstance($type, $token = null) {
    	...
    	//dump(class_exists('Org\ThinkSDK\sdk\\'.$name));
    	//判断是否开启命名空间,如果开启命名空间,就使用命名空间模式
    	if (C('APP_USE_NAMESPACE')){
    		$name = 'Org\ThinkSDK\sdk\\'.$name;
    	}else {
    		//如果不使用命名空间模式,就直接导入
