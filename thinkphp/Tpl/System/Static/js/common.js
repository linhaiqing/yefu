...
$(function(){
	...
	if($('#myqcorm').length>0){
		...
		/* 检测密码强度 */
		var check_pwd = $('#myqcorm #check_pwd');
		...
		if(frame_show){
			$.each($('#myqcorm td'),function(i,item){
			...
			$.each($('#myqcorm').find("input,select,textarea").not(":submit,:reset,:image,[disabled]"),function(i,item){
				if($(item).attr('tips')){
			...
			$.each($('#myqcorm .notice_tips'),function(i,item){
