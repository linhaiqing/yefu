<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{:U('Home/first')}" class="on">首次关注回复</a>
				</ul>
			</div>
			<form method="post" action="" refresh="true" enctype="multipart/form-data" >
				<table cellpadding="0" cellspacing="0" class="table_form" width="100%">
					<tr>
						...
						<label><input type="radio" name="type" class="type" value="3" <if condition="$first['type'] eq 3">checked</if>>本站推荐的{$config.group_alias_name}</label>
						<label><input type="radio" name="type" class="type" value="4" <if condition="$first['type'] eq 4">checked</if>>本站推荐的{$config.waimai_alias_name}店铺</label>
						</td>
					</tr>
					<tr class="class_0" <if condition="$first['type'] neq 0">style="display:none"</if>>
						<th width="160">回复内容　</th>
						<td><textarea rows="20" cols="45" name="content" id="content">{$first.content}</textarea></td>
					</tr>
					<tr class="class_1" <if condition="$first['type'] neq 1">style="display:none"</if>>
						<th width="160">回复标题　</th>
						<td><input type="text" class="input-text" name="title" id="title" value="{$first.title}"/></td>
					</tr>
					<tr class="class_1" <if condition="$first['type'] neq 1">style="display:none"</if>>
						<th width="160">内容介绍　</th>
						<td><textarea rows="4" cols="25" name="info" id="info">{$first.info}</textarea></td>
					</tr>
					<tr class="class_1" <if condition="$first['type'] neq 1">style="display:none"</if>>
						<th width="160">外链URL　</th>
						<td>
							<input type="text" class="input-text" name="url" id="url" value="{$first.url}" style="width:200px;" placeholder="外链接url如：http://www.baidu.com" validate="maxlength:200,url:true"/>
					...
					<tr class="class_1" <if condition="$first['type'] neq 1">style="display:none"</if>>
						...
						<td><input type="file" class="input-text" name="pic" id="pic" value="{$first.pic}"/></td>
					</tr>
					<if condition="$first['pic']">
						<tr class="class_1" <if condition="$first['type'] neq 1">style="display:none"</if>>
							<th width="160"></th>
							<td><img src="{$first.pic}" width="280" height="180"></td>
						</tr>
					</if>
					...
					<tr class="class_2" <if condition="$first['type'] neq 2">style="display:none"</if>>
						...
						<label><input type="radio" name="fromid" value="2" <if condition="$first['fromid'] eq 2">checked</if>>{$config.group_alias_name}首页</label>
						<label><input type="radio" name="fromid" value="3" <if condition="$first['fromid'] eq 3">checked</if>>{$config.meal_alias_name}首页</label>
						</td>
					</tr>
