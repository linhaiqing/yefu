<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{:U('Home/group')}" class="on">{$config.group_alias_name}回复回复</a>
				</ul>
			</div>
			<form method="post" action="" refresh="true" enctype="multipart/form-data" >
				<table cellpadding="0" cellspacing="0" class="table_form" width="100%">
					<tr>
						<th width="160">关键词　</th>
						<td>{$config.group_alias_name}  —— 当用户输入该关键词时，将会触发此回复。</td>
					</tr>
					<tr>
						<th width="160">回复标题　</th>
						<td><input type="text" class="input-text" name="title" id="title" value="{$info.title}" /></td>
					</tr>
					<tr>
						<th width="160">内容介绍　</th>
						<td><textarea rows="4" cols="25" name="info" id="info">{$info.info}</textarea></td>
					</tr>
					<tr>
						<th width="160">回复图片　</th>
						<td><input type="file" class="input-text" name="pic" id="pic" value="{$info.pic}"/></td>
					</tr>
