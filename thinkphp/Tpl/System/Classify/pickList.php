		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
					<if condition="empty($now_category)">
						<a href="{:U('Classify/pickList')}" class="on">主分类列表</a>|
					<else/>
						<a href="{:U('Classify/pickList')}">主分类列表</a>|
						<a href="{:U('Classify/pickList',array('fcid'=>$fcid,'pfcid'=>$pfcid))}" class="on">{$now_category.cat_name} - 子分类列表</a>|
						 <a <if condition="$now_category['subdir'] eq 1"> href="{:U('Classify/pickList',array('fcid'=>0,'pfcid'=>0))}" <elseif condition="$now_category['subdir'] eq 2"/> href="{:U('Classify/pickList',array('fcid'=>$now_category['fcid'],'pfcid'=>0))}" </if>>返回上级目录</a>
					</if>
			</div>
							...
							<if condition="is_array($category_list)">
								<volist name="category_list" id="vo">
									<tr>
										...
										<td>{$vo.cat_url}</td>
										<if condition="!isset($now_category)">
											<td><a href="{:U('Classify/pickList',array('fcid'=>$vo['cid'],'pfcid'=>$vo['fcid']))}">查看子分类</a>
										</td>
 									  <else/>
										...
										<else/>
										<a class="btn" href="javascript:void(0);" onclick="window.top.artiframe('{:U('Classify/getServerCy',array('cid'=>$vo['cid'],'param'=>$param))}','设置更新目录',800,550,true,false,false,submitbtn,'Csubmit',true);">设置更新分类</a>
										</if>
										</td>
										<td>
										<if condition="!$ishavefield">
										...
										<else/>
										<a class="btn"  <if condition="empty($vo['pickset'])"> href="javascript:alert('请您先设置一下更新分类');"<else/>href="{:U('Classify/updateFiield',array('cid'=>$vo['cid'],'fcid'=>$vo['fcid']))}"</if>>设置更新字段</a>
										</td>
										</if>
										</if>
										...
										<else/>
										<a href="{:U('Classify/updatedata',array('cid'=>$vo['cid'],'fcid'=>$vo['fcid']))}">更 新</a>
										</if>
										</td>
									</tr>
								</volist>
