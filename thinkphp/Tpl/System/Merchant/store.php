		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					...
					<a href="{:U('Merchant/store',array('mer_id'=>$merchant['mer_id']))}" class="on">{$merchant.name} - 店铺列表</a>|
					<a href="javascript:void(0);" onclick="window.top.artiframe('{:U('Merchant/store_add',array('mer_id'=>$merchant['mer_id']))}','添加店铺',520,440,true,false,false,addbtn,'store_add',true);">添加店铺</a>
				</ul>
			</div>
					...
					<table width="100%" cellspacing="0">
						...
						<thead>
							<tr>
								...
								<th>最后编辑时间</th>
								<th>平台点击数</th>
								<th>{$config.meal_alias_name}</th>
								<th>{$config.group_alias_name}</th>
								<if condition="$config['store_open_waimai']"><th>{$config.waimai_alias_name}</th></if>
								<th>状态</th>
							...
							<if condition="is_array($store_list)">
								<volist name="store_list" id="vo">
									<tr>
										...
										<td class="textcenter"><a href="javascript:void(0);" onclick="window.top.artiframe('{:U('Merchant/store_edit',array('store_id'=>$vo['store_id'],'frame_show'=>true))}','查看店铺信息',520,440,true,false,false,false,'detail',true);">查看</a> | <a href="javascript:void(0);" onclick="window.top.artiframe('{:U('Merchant/store_edit',array('store_id'=>$vo['store_id']))}','编辑店铺信息',520,440,true,false,false,editbtn,'store_add',true);">编辑</a> | <a href="javascript:void(0);" class="delete_row" parameter="store_id={$vo.store_id}" url="{:U('Merchant/store_del')}">删除</a></td>
									</tr>
								</volist>
