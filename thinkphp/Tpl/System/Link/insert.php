...
<div class="mright">
<if condition="$m['sub']"><a href="<?php if (!$m['linkurl']){?>?g=System&c=Link&a={$m.module}&iskeyword=<?php echo intval($_GET['iskeyword']);?><?php }else{echo $m['linkurl'];}?>">详细</a></if>
<if condition="$m['canselected']"><a href="###" onclick="returnHomepage('<?php if (!intval($_GET['iskeyword'])){?>{$m.linkcode}<?php }else{?>{$m.keyword}<?php }?>')" style="margin-left:14px;">选中</a></if>
</div>
