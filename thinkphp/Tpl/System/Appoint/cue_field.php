<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{:U('Appoint/cue_field',array('cat_id'=>$now_category['cat_id']))}" class="on">填写项列表</a>|
					<a href="javascript:void(0);" onclick="window.top.artiframe('{:U('Appoint/cue_field_add',array('cat_id'=>$now_category['cat_id']))}','添加填写项',460,250,true,false,false,addbtn,'add',true);">添加填写项</a>
				</ul>
			</div>
							...
							<if condition="is_array($now_category['cue_field'])">
								<volist name="now_category['cue_field']" id="vo">
									<tr>
										...
										<td class="textcenter"><a href="javascript:void(0);" class="delete_row" parameter="cat_id={$now_category.cat_id}&name={$vo.name}" url="{:U('Appoint/cue_field_del')}">删除</a></td>
									</tr>
								</volist>
							<else/>
