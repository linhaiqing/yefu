...
class Group_orderModel extends Model{
		...
		if(empty($now_order)){
			...
			if($this->where($condition_group_order)->data($data_group_order)->save()){
				...
				if(!empty($now_user['openid'])){
					//上级分享佣金
					$now_user_spread = D('User_spread')->field('`spread_openid`')->where(array('openid'=>$now_user['openid']))->find();
					if(!empty($now_user_spread)){						
						$spread_user = D('User')->get_user($now_user_spread['spread_openid'],'openid');
						if($spread_user && C('config.user_spread_rate')){
							$spread_money = round(($balance_pay+$data_group_order['payment_money'])*C('config.user_spread_rate')/100,2);
							D('User_spread_list')->data(array('uid'=>$spread_user['uid'],'spread_uid'=>0,'get_uid'=>$now_user['uid'],'money'=>$spread_money,'order_type'=>'group','order_id'=>$now_order['order_id'],'third_id'=>$now_order['group_id'],'add_time'=>$_SERVER['REQUEST_TIME']))->add();
							// D('User')->add_money($spread_user['uid'],$spread_money,'推广用户 '.$now_user['nickname'].' 购买 ID为 '.$now_order['group_id'].' 的商品 获得的佣金');
						}
						
						//顶级分享佣金
						$first_user_spread = D('User_spread')->field('`spread_openid`')->where(array('openid'=>$now_user_spread['spread_openid']))->find();
						if(!empty($first_user_spread)){
							$first_spread_user = D('User')->get_user($first_user_spread['spread_openid'],'openid');
							if($first_spread_user && C('config.user_first_spread_rate')){
								$spread_money = round(($balance_pay+$data_group_order['payment_money'])*C('config.user_first_spread_rate')/100,2);
								D('User_spread_list')->data(array('uid'=>$first_spread_user['uid'],'spread_uid'=>$spread_user['uid'],'get_uid'=>$now_user['uid'],'money'=>$spread_money,'order_type'=>'group','order_id'=>$now_order['order_id'],'third_id'=>$now_order['group_id'],'add_time'=>$_SERVER['REQUEST_TIME']))->add();								
	...
	{
		$count = $this->where(array('group_id' => $group_id, 'uid' => $uid, 'paid' => 1))->sum('num');
		$group = D('Group')->get_group_by_groupId($group_id);
		
		$max_num = $group['once_max'];
		
		$min_num = $group['once_min'];

		//商品有库存限制的时候
		if ($group['count_num'] > 0) {
			$k_num = $group['count_num'] - $group['sale_count']; //库存
			if ($k_num < $min_num) {//库存等于销售量的时候不能买
				return array('errcode' => 1, 'msg' => '您已经没有团购该商品的机会了');
			} else {
				if ($max_num > 0) {
					$my_num = $max_num - $count;//我的剩余得到份数
					if ($my_num < $min_num) return array('errcode' => 1, 'msg' => '您已经没有团购该商品的机会了');
					if ($num > $max_num) {
						if ($k_num < $my_num && $k_num < $max_num) {
							return array('errcode' => 1, 'msg' => '您最多能购买' . $k_num . '份');
						} elseif ($my_num < $max_num) {
							return array('errcode' => 1, 'msg' => '您最多能购买' . $my_num . '份');
						} else {
							return array('errcode' => 1, 'msg' => '您最多能购买' . $max_num . '份');
						}
					}
				} else {
					if ($num > $k_num) {
