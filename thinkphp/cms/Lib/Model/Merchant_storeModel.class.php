...
class Merchant_storeModel extends Model
{
	...
	public function get_list_by_option($area_id = 0, $circle_id = 0, $order = 'store_id', $lat = 0, $long = 0, $cat_id = 0, $cat_fid = 0, $store_type = 2)
	{
		...
		$sql_count = "SELECT count(1) as count FROM " . C('DB_PREFIX') . "merchant_store AS s INNER JOIN " . C('DB_PREFIX') . "merchant_store_meal as m ON m.store_id=s.store_id WHERE {$condition_where}";
		...
		$sql = "SELECT s.*, m.* {$juli} FROM " . C('DB_PREFIX') . "merchant_store AS s INNER JOIN " . C('DB_PREFIX') . "merchant_store_meal as m ON m.store_id=s.store_id WHERE {$condition_where} ORDER BY {$order} LIMIT {$p->firstRow}, {$p->listRows}";
		$res = $mod->query($sql);
		// dump($mod);
		$ids = array();
		foreach ($res as $r) {
...
// 		$condition_table = array(C('DB_PREFIX') . 'merchant_store' => 's', C('DB_PREFIX') . 'merchant_store_meal' => 'm');
		...
		$condition_where = "`me`.`city_id`='".C('config.now_city')."' AND s.have_meal=1 AND s.status=1 AND s.store_id=m.store_id";
		if (is_array($store_type)) {
			$condition_where .= " AND s.store_type IN (" . implode(',', $store_type) . ")";
		} else {
			$condition_where .= "  AND s.store_type={$store_type}";
...
// 					break;
	...
	{
		$condition_where = "`me`.`city_id`='".C('config.now_city')."' AND s.have_meal=1 AND s.status=1";
		$Model = new Model();
		if ($lat && $long) {
