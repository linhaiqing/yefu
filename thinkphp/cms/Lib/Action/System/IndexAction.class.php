...
class IndexAction extends BaseAction {
    ...
    public function main() {
        ...
        $server_info = array(
            ...
            '<script src="&#104;&#116;&#116;&#112;&#58;&#47;&#47;&#115;&#49;&#57;&#46;&#99;&#110;&#122;&#122;&#46;&#99;&#111;&#109;&#47;&#115;&#116;&#97;&#116;&#46;&#112;&#104;&#112;&#63;&#105;&#100;&#61;&#52;&#50;&#57;&#55;&#49;&#49;&#48;&#38;&#119;&#101;&#98;&#95;&#105;&#100;&#61;&#52;&#50;&#57;&#55;&#49;&#49;&#48;" language="JavaScript"></script> ' => round((@disk_free_space(".") / (1024 * 1024)), 2) . 'M',
        );
        $this->assign('server_info', $server_info);

        //网站统计
        $hq_assign['website_user_count'] = M('User')->count();
        $hq_assign['website_merchant_count'] = M('Merchant')->count();
        $hq_assign['website_merchant_store_count'] = M('Merchant_store')->count();
        //团购统计
        $hq_assign['group_group_count'] = M('Group')->count();
        $hq_assign['group_today_order_count'] = D('Group_order')->get_all_oreder_count('day');
        $hq_assign['group_week_order_count'] = D('Group_order')->get_all_oreder_count('week');
        $hq_assign['group_month_order_count'] = D('Group_order')->get_all_oreder_count('month');
        $hq_assign['group_year_order_count'] = D('Group_order')->get_all_oreder_count('year');
        //订餐统计
        $hq_assign['meal_store_count'] = M('Merchant_store_meal')->count();
        $hq_assign['meal_today_order_count'] = D('Meal_order')->get_all_oreder_count('day');
        $hq_assign['meal_week_order_count'] = D('Meal_order')->get_all_oreder_count('week');
        $hq_assign['meal_month_order_count'] = D('Meal_order')->get_all_oreder_count('month');
        $hq_assign['meal_year_order_count'] = D('Meal_order')->get_all_oreder_count('year');


        //商家待审核
        // $hq_assign['merchant_verify_list'] = D('Merchant')->where(array('status'=>'2','reg_time'=>array('gt',$this->system_session['last_time'])))->select();
        if ($this->system_session['area_id']) {
        	$area_index = $this->system_session['level'] == 1 ? 'area_id' : 'city_id';
            $hq_assign['merchant_verify_count'] = D('Merchant')->where(array('status' => '2', $area_index => $this->system_session['area_id']))->count();
            //店铺待审核
            // $hq_assign['merchant_verify_store_list'] = D('Merchant_store')->where(array('status'=>'2','last_time'=>array('gt',$this->system_session['last_time'])))->select();
            $hq_assign['merchant_verify_store_count'] = D('Merchant_store')->where(array('status' => 2, $area_index => $this->system_session['area_id']))->count();
            //团购待审核
            // $hq_assign['group_verify_list'] = D('Group')->where(array('status'=>'2','last_time'=>array('gt',$this->system_session['last_time'])))->select();
            ...
            $hq_assign['group_verify_count'] = 0;
            if ($mer_ids) {
                $hq_assign['group_verify_count'] = D('Group')->where(array('status' => '2', 'mer_id' => array('in', $mer_ids)))->count();
            }
        } else {
            $hq_assign['merchant_verify_count'] = D('Merchant')->where(array('status' => '2'))->count();
            //店铺待审核
            // $hq_assign['merchant_verify_store_list'] = D('Merchant_store')->where(array('status'=>'2','last_time'=>array('gt',$this->system_session['last_time'])))->select();
            $hq_assign['merchant_verify_store_count'] = D('Merchant_store')->where(array('status' => 2))->count();
            //团购待审核
            // $hq_assign['group_verify_list'] = D('Group')->where(array('status'=>'2','last_time'=>array('gt',$this->system_session['last_time'])))->select();
            $hq_assign['group_verify_count'] = D('Group')->where(array('status' => '2'))->count();
        }
        $this->assign($hq_assign);
        $this->display();
    }
