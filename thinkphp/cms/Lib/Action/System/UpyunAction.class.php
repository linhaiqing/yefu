...
class UpyunAction extends BaseAction
{
	...
	public function _initialize() {
		...
		$this->siteUrl = $this->siteUrl ? $this->siteUrl : C('config.site_url');
		$this->token = $this->system_session['id'];
	}
	...
	public function upload()
	{
		if ($this->upload_type=='upyun'){
			...
			if (IS_POST) {
				...
				echo '<script>location.href="/admin.php?g=System&c=Upyun&a=upload&error='.$return['error'].'&msg='.$return['msg'].'";</script>';
			} else {
				if (!isset($_GET['from'])) {
					$this->display('local');
				} else {
...
//导入excel会员卡
	...
	{
		$path = $path ? $path : 'card';
// 		$param = array('size' => 5, 'thumbRemoveOrigin' => true);
// 		if (isset($_POST['width']) && intval($_POST['width'])) {
// 			$param['thumb'] = true;
// 			$param['thumbMaxWidth'] = $_POST['width'];
// 			$param['thumbMaxHeight'] = $_POST['height'];
// 			$thumb = 1;
// 		}
// 		$image = D('Image')->handle($this->system_session['id'], $path, 0, $param);
// 		if ($image['error']) {
// 			return $image;
// 		} else {
...
// 		}
		...
		$upload->savePath =  $path ? './upload/' . $path . '/'  . $firstLetter . '/' . $this->token . '/' : './upload/' . $firstLetter . '/' . $this->token . '/';// 设置附件上传目录
		$rootpath = $path ? "/upload/{$path}" : '/upload';
		if (!file_exists($_SERVER['DOCUMENT_ROOT'] . $rootpath) || !is_dir($_SERVER['DOCUMENT_ROOT'] . $rootpath)) {
			mkdir($_SERVER['DOCUMENT_ROOT'] . $rootpath, 0777);
		}
